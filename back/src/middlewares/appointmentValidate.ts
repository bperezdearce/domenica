...
import IAppointmentDto from '../dto/IAppointmentsDto';
...
const appointmentValidate = (
    ...
    try {   
        if (!date) throw new Error("El campo fecha es requerido");
        ...
        if (!time) throw new Error("El campo hora es requerido");
        const validTimes = [
            "12:00",
            "12:30",
            "13:00",
        ...
        if (!peopleNumber) throw new Error("El campo número de personas es requerido");
        if (typeof peopleNumber !== "number") throw new Error("El campo número de personas debe ser un número");
